...
def mergable(w1, w2):
    if w1["label"] == w2["label"]:
        threshold = 7
        if (
            abs(w1["box"][1] - w2["box"][1]) < threshold
            or abs(w1["box"][-1] - w2["box"][-1]) < threshold
        ):
            return True
        return False
    return False
...
def main():
    ...
    inference_processor = LayoutLMv2Processor.from_pretrained(
    ...
    special_tokens = [
        inference_processor.tokenizer.cls_token_id,
        inference_processor.tokenizer.sep_token_id,
        inference_processor.tokenizer.pad_token_id,
    ]
    ...
    predictions = [
        ...
        if not (raw_input_ids[i] in special_tokens)
    ]
    actual_boxes = [
        box
    ...
    for word in words:
        ...
        if word_labels != []:
            word_tagging = (
                word_labels[0] if word_labels[0] != "O" else word_labels[-1]
            )
        else:
            word_tagging = "O"
    ...
    filtered_words = [
        ...
        if word["word_tagging"] != "O"
    ]

    merged_taggings = []
    for i, curr_word in enumerate(filtered_words):
        skip = False
        neighbors = lambda word: [
            neighbor for neighbor in filtered_words if mergable(word, neighbor)
        ]
    ...
    for i, merged_tagging in enumerate(merged_taggings):
        if len(merged_tagging) > 1:
            ...
            merging_box = [
                merged_tagging[0]["box"][0] - 5,
                merged_tagging[0]["box"][1] - 10,
                merged_tagging[-1]["box"][2] + 5,
                merged_tagging[-1]["box"][3] + 10,
            ]
            ...
            new_word["words"] = [
                {"box": word["box"], "text": word["text"]}
                for word in merged_tagging
            ]
    ...
    label2color = {
        f"{label}": f"rgb({random_color()[0]},{random_color()[1]},{random_color()[2]})"
        for label in unique_taggings
    }
    ...
    with open(
        f"{output_path}/jsonOutput.json", "w", encoding="utf8"
    ) as outfile:

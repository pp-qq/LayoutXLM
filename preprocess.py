...
if __name__ == "__main__":
    ...
    if zip_dir_name:
        files["train_box"] = read_text_file(
            os.path.join(os.curdir, f"{zip_dir_name}\\{zip_dir_name}_box.txt")
        )
        files["train_image"] = read_text_file(
            os.path.join(
                os.curdir, f"{zip_dir_name}\\{zip_dir_name}_image.txt"
            )
        )
        files["train"] = read_text_file(
            os.path.join(os.curdir, f"{zip_dir_name}\\{zip_dir_name}.txt")
        )
    else:
        for f in os.listdir():
    ...
    class data_config:
        labels = np.unique(
            [tag for doc_tag in ner_tags for tag in doc_tag]
        ).tolist()
    ...
    dataset_dict = {
        ...
        "words": words,
        "bboxes": [
            [list(map(int, bbox.split())) for bbox in doc] for doc in bboxes
        ],
        "ner_tags": [
            [data_config.label2id[tag] for tag in ner_tag]
